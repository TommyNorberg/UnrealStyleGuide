# Style Guide for Unreal Projects
...
There are a few different ways you can name things. In most cases we will use PascalCase, but there are exceptions to this. Here are the most common casing types:
...
<a name="0.1"></a>
### 0.1 A team without a style guide is no team of mine.
...
<a name="0.2"></a>
### 0.2 All structure, assets, and code in our projects should look like a single person created it, no matter how many people contributed.
...
When working within a team it is far easier to help and to ask for help when people are consistent. Nobody likes to help untangle someone's Blueprint spaghetti or deal with assets with names they can't understand.
...
* Be very very careful with AI generated art as that is currently a trademark greyzone
...
Depending on how your asset variants are made, you can chain together variant names. For example, if you are creating flooring assets you should use the base name `Flooring` with chained variants such as `SM_Flooring_Marble_01`, `SM_Flooring_Maple_02`, `SM_Flooring_Tile_Squares_01`.
...
| ----------------------- | ---------- | ---------- | -------------------------------- |
| Level / Map             | L_         |            | 					 |
| Level (Gym)             | L_Gym_     |            | levels for testing purpouses     |
| Level (Persistent)      |            | _P         | might be deprecated workflow     |
| Level (Audio)           |            | _Audio     | might be deprecated workflow     |
| Level (Lighting)        |            | _Lighting  | might be deprecated workflow     |
| Level (Geometry)        |            | _Geo       | might be deprecated workflow     |
| Level (Gameplay)        |            | _Gameplay  | might be deprecated workflow     |
| Blueprint               | BP_        |            |                                  |
| Material                | M_         |            |                                  |
| Static Mesh             | SM_        |            |                                  |
| Skeletal Mesh           | SK_        |            |                                  |
...
| Asset Type              | Prefix     | Suffix     | Notes                            |
| ----------------------- | ---------- | ---------- | -------------------------------- |
| Blueprint               | BP_        |            | note: We do not mark child BPs in any extra way  |
| Parent Blueprint        | BPP_       |            | Blueprints that have children and never should be used directly  |
| Data Only Blueprint     | BPO_       |            | Blueprints that do not have scripts, in most cases a child BP |
| Blueprint Component	  | BPC_       |            |                                   |
| Character Blueprint	  | CBP_       |            |                                   |
| Player Controller	  | PC_       |            |                                   |
| Blueprint Interface     | BPI_       |            |                                  |
| Enumeration             | Enum_      |            |                                  |
| Structure               | Struct_    |            |                                  |
| Event Dispatchers       | Dispatcher_ |           |                                  |
| Widget Blueprint        | W_        |            |                                  |
...
| Texture (Packed)        | T_         | _ORMH         | See notes below about [packing](#anc-textures-packing). |
| Texture Cube            | TC_        |            |                                  |
| Media Texture           | MT_        |            |                                  |
| Render Target           | RT_        |            |                                  |
...
It is common practice to pack multiple layers of texture data into one texture. An example of this is packing Ambient Occlusion, Roughness, Metalness and Heightmap together as the Red, Green, Blue and alpha channels of a texture respectively. To determine the suffix, simply stack the given suffix letters from above together, e.g. `_ORMH`.

> If there is another variation of texturechannels then the suffix `_M` should be used
...
<a name="anc-GAS"></a>
<a name="1.2.13"></a>
### 1.2.12 Gameplay Ability System

| Asset Type              | Prefix     | Suffix     | Notes                            |
| ----------------------- | ---------- | ---------- | -------------------------------- |
| Gameplay Abilities      | GA_        |            |                                  |
| Gameplay Abilities      | GA_        | _Agent     | Earmarked to NOT be used by Player  |
| Gameplay Effects        | GE_        |            |                                  |
| Gameplay Cue Burst      | GCB_       |            | A one-off effect                |
| Gameplay Cue Loop      | GCL_       |            | A looping effect                |
...
<pre>
|-- Content
	|-- P01 (The project callsign and extra root)
		|-- Audio
		|-- Effects
			|-- AnimNotifiers
			|-- Blueprints
		...
		|-- GAS
			...
			|-- Cues
				...
				|-- Hits
				|-- Misc
			|-- MessageStructs (structs used for gameplay messages)
		|-- Localization
			|-- EngineOverrides
			|-- Game
		|-- Player
		...
		|-- UI
			...
			|-- Settings
	|-- World
		|-- Tools (BP worldbuilding script tools)
		|-- Meshes (worldbuilding meshes that are 'common' and could exist in any level)
			|-- Kits
				|-- StandardHouse
				|-- Castle
				|-- Cave
		...
		|-- Textures
|-- Plugins
	|-- P01_GymLevels (all the test maps)
	|-- P01_CoreLevels (all the levels and art for the base game)
		|-- Levels
		|-- Cinematics
	|-- P01_CoreAgents (all the enemies and NPCs for the base game)
		|-- Common
			|-- Audio
			|-- Effects
		|-- CoreAI (the base ai behaviour, tasks etc)
			|-- Tasks
			|-- EQS
		|-- GAS (Gameplay ability system EXCLUSIVE to agents and never used by player)
			|-- Abilities
			|-- Cues
			|-- Effects
			|-- MessageStructs
		...
		|-- Enemies
			...
			|-- Zombie
		|-- Effects
	|-- P01_CoreItems (all the pickupable items for the base game)
		|-- Weapons
		|-- Resources (gold, wood, food, etc) 
		|-- Trinkets (rings, necklaces, etc)
		|-- Equipment (armor, helmets, boots, etc)
		|-- Misc (keys and other things that do not fit into the other folders)
		|-- Questitems (unique McGuffins for completing quests)
</pre>
...
Using other characters outside `a-z`, `A-Z`, and `0-9` such as `@`, `-`, `,`, `*`, and `#` can also lead to unexpected and hard to track issues on other platforms, source control, and weaker engineering tools. The exeption beeing the `_` sign (underscore) that is heavily used so separate prefix with main body (BP_Clownhat)

<a name="2.2"></a>
<a name="structure-top-level"><a>
### 2.2 Use A Top Level Folder For Project Specific Assets

All of a project's assets should exist in a folder named after the project. For example, if your project is named 'RandomProject', _all_ of it's content should exist in `Content/Root_RandomProject`. it is good practice to keep the root name short, so `Content/Root_RP` might be even better.
...
<a name="2.2.2.1"></a>
##### 2.2.2.1 Master Material Example

For example, say you created a master material in one project that you would like to use in another project so you migrated that asset over. If this asset is not in a top level folder, it may have a name like `Content/Assets/Materials/M_Master`. If the target project doesn't have a master material already, this should work without issue.
...
The issue comes when, for example, an artist for one project created a nice generic modular set of static meshes and someone wants to include that set of static meshes in the second project. If the artist who created the assets used material instances based on `Content/Assets/Materials/M_Master` as they're instructed to, when a migration is performed there is a great chance of conflict for the previously migrated `Content/Assets/Materials/M_Master` asset.

This issue can be hard to predict and hard to account for. The person migrating the static meshes may not be the same person who is familiar with the development of both project's master material, and they may not be even aware that the static meshes in question rely on material instances which then rely on the master material. The Migrate tool requires the entire chain of dependencies to work however, and so it will be forced to grab `Content/Assets/Materials/M_Master` when it copies these assets to the other project and it will overwrite the existing asset.
...
### 2.4 All Map[<sup>*</sup>](#terms-level-map) Files Belong In A Folder Called Levels

Level files are incredibly special and it is common for every project to have its own level naming system, especially if they work with sub-levels or streaming levels. No matter what system of level organization is in place for the specific project, all levels should belong in `/Content/Root_ProjectName/Assets/Levels`.

Being able to tell someone to open a specific level without having to explain where it is is a great time saver and general 'quality of life' improvement. It is common for levels to be within sub-folders of `Levels`, such as `Levels/Campaign1/` or `Levels/Arenas`, but the most important thing here is that they all exist within a `levels plugin`.
...
All booleans should be named to phrase the variable as a question, such as `Is`. There should not be a `?` included.

Example: `bIsDead` and `bIsHostile`.
...
#### 3.3.1.4 Custom Event Handlers Should Start With `On`

Any function that handles an event should start with `On` and continue to follow [the verb rule](#bp-funcs-naming-verbs). The verb may move to the end however if past-tense reads better.
